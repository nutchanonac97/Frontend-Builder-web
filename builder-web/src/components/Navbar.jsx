...
import { Menu, X, Home, Phone, Sun, Moon, ChevronDown, Globe, Settings } from 'lucide-react';
...
const Navbar = ({ isDark, toggleTheme }) => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 backdrop-blur-xl transition-all duration-500 ${isDark
        ? 'bg-slate-900/60'
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8">
        <div className="flex items-center h-16">
          ...
          <Link to="/" className="flex items-center gap-2 cursor-pointer z-50 shrink-0">
            <div className={`p-2 rounded-lg shadow-lg transition-all duration-300 ${isDark
          ...
          <div className="hidden lg:flex items-center gap-3 shrink-0">
            <Link
              to="/contact"
              className={`px-4 py-1.5 rounded-full transition-all flex items-center gap-2 shadow-lg text-sm ${isDark
            ...
            <div className="relative" ref={langRef}>
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-all ${isDark
              ...
              {langOpen && (
                <div className={`absolute right-0 top-full mt-2 w-40 rounded-xl overflow-hidden shadow-xl border animate-in fade-in slide-in-from-top-2 duration-200 ${isDark
            ...
            <Link
              to="/config"
              className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${isDark
            ...
            <button
              onClick={toggleTheme}
              className={`relative w-11 h-6 rounded-full transition-all duration-300 flex items-center ${isDark
                  ? 'bg-slate-700/80 hover:bg-slate-600'
                  : 'bg-slate-200 hover:bg-slate-300'
                }`}
              title={isDark ? t('theme.light') : t('theme.dark')}
            >
              <div className={`absolute w-4 h-4 rounded-full shadow-md flex items-center justify-center transition-all duration-300 ${isDark
          ...
          <div className="lg:hidden z-50 flex items-center gap-1 ml-auto">
            ...
            <button
              ...
              className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 text-base ${isDark
      ...
      {/* Mobile Menu Overlay */}
      {isMenuOpen && (
        <div className={`lg:hidden absolute top-16 left-0 w-full border-t shadow-xl p-4 sm:p-6 flex flex-col gap-2 sm:gap-4 animate-in slide-in-from-top-5 max-h-[calc(100dvh-4rem)] overflow-y-auto ${isDark
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={`text-base sm:text-lg font-medium py-1.5 sm:py-2 border-b ${isDark ? 'border-slate-700' : 'border-gray-50'
                } ${isActive(link.path)
                  ? 'text-orange-500'
          ...
          <Link
            to="/config"
            onClick={() => setIsMenuOpen(false)}
            className={`text-base sm:text-lg font-medium py-1.5 sm:py-2 border-b flex items-center gap-2 ${isDark ? 'border-slate-700' : 'border-gray-50'
              } ${isActive('/config')
                ? 'text-orange-500'
                : isDark ? 'text-white' : 'text-slate-800'
              }`}
          ...
          <Link
            to="/contact"
            onClick={() => setIsMenuOpen(false)}
            className={`text-base sm:text-lg font-medium py-1.5 sm:py-2 ${isActive('/contact')
                ? 'text-orange-500'
                : isDark ? 'text-white' : 'text-slate-800'
              }`}
          >
